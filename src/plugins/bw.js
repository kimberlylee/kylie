...
function bwrun() {
     ...
     * 
     * @type Array.<{name: !string, size: number, timeout: number}>
     * @const
     */
    var images = [
    ...
    var impl = {
        ...
        calc_bw: function () {
            ...
            for (i = 0; i < impl.nruns; i++) {
                ...
                for (j = r.length - 1; j >= 0 && nimgs < 3; j--) {
                    ...
                    bw_c = images[j].size * 1000 / (r[j].t - impl.latency.mean);
        ...
        load_img: function (i, run, callback) {
            var url = impl.base_url + images[i].name + '?t=' + (new Date().getTime()) + Math.random(),    // Math.random() is slow, but we get it before we start the timer
                timer = 0,
                tstart = 0,
                img = new Image(),
                that = impl;
        ...
        img_loaded: function (i, tstart, run, success) {
            ...
            if (impl.results[impl.nruns - run].r[i]) {    // already called on this image
                return;
            }
        ...
        finish: function () {
            ...
            var bw = impl.calc_bw(),
                o = {
                    ...
                    lat: impl.latency.mean,
                    lat_err: parseFloat(impl.latency.stderr),
            ...
            if (!isNaN(o.bw) && o.bw > 0) {
                BOOMR.utils.setCookie(impl.cookie,
                    {
                        ba: Math.round(o.bw),
                        be: o.bw_err,
                        l: o.lat,
                    ...
                    (impl.user_ip ? impl.cookie_exp : 0)
                );
            }
