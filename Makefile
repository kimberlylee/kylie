...
perf-$(VERSION).$(DATE).js: perf-$(VERSION).$(DATE)-debug.js
	...
	echo "*/" >> $@
	echo ")]}" | cat - $(MAP_FILE_NAME) > /tmp/$(MAP_FILE_NAME).tmp && mv /tmp/$(MAP_FILE_NAME).tmp $(MAP_FILE_NAME)
	echo "done"
	echo
